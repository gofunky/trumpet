...
module.exports = function (selector) {
    ...
    var matchers = [ new Match(selector, {}) ];
    matchers[0].on('attribute', onattribute);
    matchers[0].on('open', onopen);
    matchers[0].on('fork', onfork);
    
    group.at = function (kind, node) {
        for (var i = 0; i < matchers.length; i++) {
            matchers[i].at(kind, node);
        }
    };
    return group;
    ...
    function onopen (node) {
        if (matchers.length > 1) {
            ...
            matchers = [ matchers[ix] ];
        }
        group.emit('open', node);
    }
    
    function onattribute (x) {
        group.emit('attribute', x);
    }
...
Match.prototype.at = function (kind, node) {
    ...
    if (!sel && kind === 'tag-begin') return this.unmatch();
